...
There is no installable Home Assistant integration, registered panel route or HACS package yet. The future panel must implement **NikaS Specialized Panel UI Standard v1.7** directly; the current compliance audit is in `docs/NIKAS_SPECIALIZED_PANEL_COMPLIANCE.md` and the machine-readable decision is in `docs/SPECIALIZED_PANEL_READINESS.json`.
